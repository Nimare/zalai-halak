...
import {
  ...
  buildOptions,
  acceptsAnswer,
  beginnerConfusionGroups,
  beginnerDistractorWeight,
} from '../lib/game-rules.ts';

const data = JSON.parse(
  fs.readFileSync(new URL('../data/fishes.json', import.meta.url), 'utf8'),
);
const fishes = data.fishes;
const [header, ...lines] = fs
  .readFileSync(new URL('../data/species-table.tsv', import.meta.url), 'utf8')
  .trim()
  .split(/\r?\n/);
...
for (const line of lines) {
  ...
  for (const [field, value] of Object.entries(row))
    assert.equal(String(fish[field]), value, `${fish.id}: ${field}`);
}
for (const fish of fishes) {
  ...
  assert.ok(
    fishFacts[fish.id]?.identification,
    `${fish.id}: missing recognition text`,
  );
  assert.ok(['kezdő', 'szakértő'].includes(fish.category));
  assert.ok(data.originLegend[fish.origin]);
  for (const field of ['factSource', 'imageSource', 'licenseUrl'])
    assert.ok(['https:', 'http:'].includes(new URL(fish[field]).protocol));
  const bytes = fs.readFileSync(
    new URL(`../public${fish.image}`, import.meta.url),
  );
  assert.ok(
    ['ffd8ff', '89504e'].includes(bytes.subarray(0, 3).toString('hex')),
    `${fish.id}: invalid image`,
  );
...
for (let run = 0; run < 25; run++) {
  ...
  for (const fish of beginner) {
    ...
    assert.ok(
      options.every((name) => {
        ...
        return (
          option.category === 'kezdő' ||
          beginnerDistractorWeight(fish.id, option.id) > 1
        );
      }),
    );
  }
  ...
  assert.ok(
    expert.every((f) =>
      fishPool(fishes, 'expert').some((entry) => entry.id === f.id),
    ),
  );
}
for (const group of beginnerConfusionGroups) {
  ...
  for (const id of group.ids)
    assert.ok(
      fishes.some((f) => f.id === id),
      `Unknown confusion-group ID: ${id}`,
    );
}
const karika = fishes.find((f) => f.id === 'karikakeszeg');
const relatedKeszegIds = [
  'deverkeszeg',
  'laposkeszeg',
  'bagolykeszeg',
  'szilvaorru-keszeg',
...
for (const id of relatedKeszegIds) {
  assert.ok(
    beginnerDistractorWeight(karika.id, id) >
      beginnerDistractorWeight(karika.id, 'csuka'),
  );
  assert.equal(
    beginnerDistractorWeight(karika.id, id),
    beginnerDistractorWeight(id, karika.id),
  );
}
...
try {
  ...
  for (const id of relatedKeszegIds) {
    ...
    assert.ok(
      counts.get(name) > counts.get('Csuka') * 2,
      `${name} should appear substantially more often than Csuka`,
    );
  }
  assert.ok(
    correctPositions.every(
      (count) => count > runs * 0.22 && count < runs * 0.28,
    ),
  );
  ...
  assert.equal(
    counts.get('Simatok'),
    undefined,
    'Unrelated expert fish stay excluded',
  );
  console.log(
    'Karikakeszeg distractor inclusion (%):',
    Object.fromEntries(
  ...
  for (const random of [0, 1 - Number.EPSILON]) {
    ...
    const tinyPool = fishes.filter((f) =>
      ['karikakeszeg', 'deverkeszeg', 'csuka'].includes(f.id),
    );
    assert.deepEqual(
      new Set(buildOptions(karika, tinyPool, 'beginner')),
      new Set(tinyPool.map((f) => f.nameHu)),
    );
  }
} finally {
...
for (const fish of fishes) {
  for (const answer of [fish.nameHu, ...(fish.alsoKnownAsHu ?? [])])
    assert.ok(acceptsAnswer(answer, fish, fishes));
  assert.equal(acceptsAnswer('', fish, fishes), false);
  for (const other of fishes.filter((f) => f.id !== fish.id))
    assert.equal(
      acceptsAnswer(other.nameHu, fish, fishes),
      false,
      `${other.nameHu} incorrectly accepted for ${fish.nameHu}`,
    );
}
assert.ok(
  acceptsAnswer(
    '  vorosszarnyu keszeg ',
    fishes.find((f) => f.id === 'vorosszarnyu-keszeg'),
    fishes,
  ),
);
assert.ok(
  acceptsAnswer(
    'szivarvanyos okel',
    fishes.find((f) => f.id === 'szivarvanyos-okle'),
    fishes,
  ),
);
console.log(
  'Rendben: 58 táblázatsor + nyurgaponty, 59 kép és felismerési leírás; kezdő/szakértő szűrés, válaszlehetőségek, névváltozatok.',
);
